...
router.get('/', async (req, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ error: 'An error occurred while retrieving skills.' });
  }
});
...
router.post('/', authenticateToken, async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  if (!name || !description || level === undefined || !category) {
    res.status(400).json({ error: 'Missing required fields. name, description, level, and category are required.' });
    return;
  }

  const numericLevel = Number(level);
  if (isNaN(numericLevel) || numericLevel < 0 || numericLevel > 100) {
    res.status(400).json({ error: 'level field must be a number between 0 and 100.' });
    return;
  }

...
router.get('/', async (req, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ error: 'An error occurred while retrieving experience.' });
  }
});
...
router.post('/', authenticateToken, async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  if (!role || !company || !duration || !description) {
    res.status(400).json({ error: 'Missing required fields. role, company, duration, and description are required.' });
    return;
  }

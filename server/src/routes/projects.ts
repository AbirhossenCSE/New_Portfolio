...
router.post(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    const { title, description, image, tech, live, github, featured, order } =
      req.body;

    if (!title || !description || !image || !tech || !live || !github) {
      res.status(400).json({
        error:
          "Missing required fields. title, description, image, tech, live, and github are required.",
      });
      return;
    }
    ...
    if (!Array.isArray(tech)) {
      res
        .status(400)
        .json({ error: "tech field must be an array of strings." });
      return;
    }

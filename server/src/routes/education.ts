...
router.get("/", async (req, res: Response): Promise<void> => {
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while retrieving education." });
  }
});
...
router.post(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    if (!year || !degree || !institution || !description) {
      res.status(400).json({
        error:
          "Missing required fields. year, degree, institution, and description are required.",
      });
      return;
    }
    ...
    try {
      ...
      res.status(500).json({
        error: "An error occurred while creating the education entry.",
      });
    }
  },
);
...
router.put(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
      ...
      res.status(500).json({
        error: "An error occurred while updating the education entry.",
      });
    }
  },
);
...
router.delete(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
      ...
      res
        .status(200)
        .json({ message: "Education entry deleted successfully." });
    } catch (error) {
      console.error("Error deleting education:", error);
      res.status(500).json({
        error: "An error occurred while deleting the education entry.",
      });
    }
  },
);
